 ...
 * @param optional nil or *BalancesUsingGETOpts - Optional Parameters:
     * @param "AccountId" (optional.String) -  口座ID 口座を識別するIDまたは、つかいわけ口座を識別するID  minLength: 12 maxLength: 29
 ...
 * @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 * @param accountId 口座ID 半角英数字 口座を識別するIDまたは、つかいわけ口座を識別するID  科目コードが以下の場合のみ受け付けます ・01&#x3D;普通預金（有利息） ・02&#x3D;普通預金（決済用）  minLength: 12 maxLength: 29
 * @param xAccessToken アクセストークン  minLength: 1 maxLength: 128
...
指定した円普通預金口座のVisaVISAデビット取引明細情報を照会します
 * @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 * @param accountId 口座ID 半角英数字 口座を識別するIDまたは、つかいわけ口座を識別するID  科目コードが以下の場合のみ受け付けます ・01&#x3D;普通預金（有利息） ・02&#x3D;普通預金（決済用）  minLength: 12 maxLength: 29
 * @param xAccessToken アクセストークン  minLength: 1 maxLength: 128
...
func (a *AccountApiService) VisaTransactionsUsingGET(ctx context.Context, accountId string, xAccessToken string, localVarOptionals *VisaTransactionsUsingGETOpts) (VisaTransactionsResponse, *http.Response, error) {
	...
	if localVarHttpResponse.StatusCode >= 300 {
		...
		if localVarHttpResponse.StatusCode == 200 {
			var v VisaTransactionsResponse
			err = a.client.decode(&v, localVarBody, localVarHttpResponse.Header.Get("Content-Type"))
			if err != nil {
				newErr.error = err.Error()
				return localVarReturnValue, localVarHttpResponse, newErr
